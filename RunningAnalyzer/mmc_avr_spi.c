...
/-------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------
Modified for use on Atmega328PB
//------------------------------------------------------------------------*/
...
#define CS_LOW()	PORTB &= ~(1<<PD2)			/* CS=low */
#define	CS_HIGH()	PORTB |= (1<<PD2)			/* CS=high */
...
#define	FCLK_SLOW()	SPCR = 0x53		/* Set slow clock (F_CPU / 128) */
...
void power_on (void)
{
	...
	/*Configure  SD Chip select pin*/
	SD_CS_PORT|=(1<<SD_CS_PIN);  //Same thing is done below
...
void power_off (void)
{
	...
	SPCR &= ~((1<<SPE)|(1<<MSTR)); /* Uninitialize SPI port */
...
DSTATUS mmc_disk_initialize (void)
{
	...
	if (send_cmd(CMD0, 0) == 1) {			/* Put the card SPI mode */
		Timer1 = 255;						/* Initialization timeout of 20000 msec */
